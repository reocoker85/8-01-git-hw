...
https://hub.docker.com/repository/docker/reocoker/custom-nginx/general
...
Контейнер остановился, так как после нажатия комбинации Ctrl-C был отпрвлен сигнал SIGINT	02 прерывание (interrupt).

![3.png](./img/3.png)
![4.png](./img/4.png)

Проблема возникла из-за того ,что при запуске контейнера пробрасывали соединение на 80 порт , на котором nginx "работает" по
умолчанию. Потом изменили настройки программы на 81 порт, но настройки docker изменены не были.

![5.png](./img/5.png)

Для нового сопоставления портов нужно изменить файлы конфигурации Docker - hostconfig.json и config.v2.json , изменив 80 порт на 81.

Для удаления запущенного контейнера используем опцию -f .
